...
function isComplete(k: Partial<DevKeys> | undefined | null): k is DevKeys {
  return !!(k?.solanaSecretKeyB58 && k.suiSecretKey && k.btcWif && k.utxopiaSeedHex);
}
 ...
 *  mount the dev signer. Every source requires all four fields. */
export function loadDevKeys(): DevKeys | null {
  ...
  const sui = process.env.NEXT_PUBLIC_DEV_SUI_SK;
  const btc = process.env.NEXT_PUBLIC_DEV_BTC_WIF;
  const seed = process.env.NEXT_PUBLIC_DEV_UTXOPIA_SEED;
  if (sol && sui && btc && seed) {
    return { solanaSecretKeyB58: sol, suiSecretKey: sui, btcWif: btc, utxopiaSeedHex: seed };
  }
  return null;
}
