...
export function StandardTransferDetails({ tx, network }: { tx: TransferTx; network?: NetworkId }) {
  return (
      ...
      <div className="grid grid-cols-2 divide-x divide-gray/10">
        ...
        <div className="p-4 space-y-2.5">
          ...
          {getTxCommitmentOutputs(tx).map((out, i) => (
            <CommitmentRow key={out.leafIndex} commitment={out.commitment!} leafIndex={out.leafIndex!} txSignature={tx.txSignature} index={i + 1} network={network} />
          ))}
        </div>
      </div>
    </div>
